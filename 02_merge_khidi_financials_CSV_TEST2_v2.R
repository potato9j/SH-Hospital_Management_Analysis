...
# 목적:
#   1) 재무상태표 60개 파일 -> 통합 wide 데이터 1개
#   2) 손익계산서 60개 파일 -> 통합 wide 데이터 1개
#   3) 재무상태표 분류ID-계정과목 매칭표 1개
#   4) 손익계산서 분류ID-계정과목 매칭표 1개
#
# 최종 출력 파일 수: 정확히 4개
#   - KHIDI_재무상태표_통합.csv
#   - KHIDI_손익계산서_통합.csv
...
# 금액 후보 열 판단 기준
# 주석번호처럼 작은 숫자 열을 금액 열로 오인하지 않도록 하는 최소 절대값 합계 기준입니다.
amount_abs_min <- 1000

# 최종 출력 폴더
# 이 폴더는 실행할 때마다 삭제 후 다시 생성됩니다.
# 따라서 이 폴더 안에는 최종 4개 파일만 남습니다.
out_dir <- file.path(data_dir, "merged_output_4files")
...
# 1) 패키지 준비 --------------------------------------------------------------
required_packages <- c(
  "readxl", "dplyr", "tidyr", "stringr", "purrr", "tibble", "readr"
)
...
read_statement_one <- function(path) {
  ...
  score_tbl <- purrr::map_dfr(candidate_cols, function(j) {
    ...
    label_rows <- seq.int(max(1, header_row - 3), min(nrow(raw_cell), header_row + 2))
  ...
  if (is.na(current_col)) {
    current_col <- amount_candidates$col_no[1]
  }

  if (is.na(previous_col)) {
    previous_col <- ifelse(nrow(amount_candidates) >= 2, amount_candidates$col_no[2], NA_integer_)
  }
